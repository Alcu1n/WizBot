...
1. **自定义AI模型选择**：
   - 在插件设置中添加自定义模型选项
   - 预设多种常用模型供快速选择
   - 增加自定义模型输入框，支持输入任意OpenAI兼容模型名称

2. **前端JavaScript修改**：
   - 更新API测试功能，支持自定义模型传递
   - 增强前端表单交互，动态显示/隐藏自定义模型输入框
   - 添加表单验证，确保自定义模型名称不为空

3. **后端处理优化**：
   - 修改API类构造函数，自动处理自定义模型参数
   - 增强API验证功能，支持测试自定义模型连接
   - 完善模型参数的安全性处理和验证
...
1. 考虑添加更多AI模型相关参数设置(如temperature、max_tokens等)
2. 实现向量知识库集成功能
3. 优化AI回复的格式化处理
4. 考虑添加多模型切换功能，支持不同场景使用不同模型
